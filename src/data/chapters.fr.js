...
export const CHAPTERS_CONTENT_FR = {
  1: {
    ...
    theory: {
      ...
      keyPoints: [
        ...
        '📍 Chemin global — macOS/Linux : ~/.claude/  ·  Windows : %USERPROFILE%\\.claude\\',
      ],
...
- Modifier \`src/legacy/\` sans avertissement`,
  ...
  2: {
    ...
    theory: {
      summary:
        "Par défaut, Claude repart de zéro à chaque session. La mémoire, c'est juste un dossier de fichiers texte qu'il lit au démarrage — si c'est écrit, il s'en souvient.",
      keyPoints: [
...
via Docker en dev. Voir docker-compose.yml.`,
  ...
  3: {
    ...
    theory: {
      summary:
        'Les Skills sont des fichiers Markdown qui définissent des commandes slash (/nom) que Claude exécute à la demande.',
      keyPoints: [
      ...
      solution: `// Fichier : .claude/settings.local.json
{
  ...
  5: {
    ...
    theory: {
      summary:
        'Une session Claude Code maintient un contexte persistant. Savoir orchestrer une session longue est une compétence clé.',
      keyPoints: [
...
cat error.log | claude -p "Analyse ces erreurs"`,
  ...
  6: {
    ...
    theory: {
      summary:
        'Sans MCP, Claude ne voit que tes fichiers. Avec MCP, il peut lire ta base de données, créer des issues GitHub, faire des recherches web... Chaque service se "branche" avec une commande.',
      keyPoints: [
      ...
      codeExamples: {
        basic: `# Filesystem — aucun token requis
...
# → Claude liste read_file, list_directory...`,
        advanced: `# GitHub — avec token d'accès
...
# "Crée une issue 'Test MCP'"`,
    ...
    challenge: {
      ...
      hint: 'La commande `claude mcp add` écrit dans `~/.claude.json` (macOS/Linux) ou `%USERPROFILE%\\.claude.json` (Windows). C\'est la seule méthode fiable — éditer settings.json manuellement ne fonctionne pas avec le CLI actuel.',
    },
  },
